...
const language_buttons = document.querySelectorAll(".lang_button")
const language_toggles = document.querySelectorAll(".language_toggle")


button.addEventListener( "click", () => {
    circle.classList.toggle("clicked");
    language_buttons.forEach(element => {
        element.classList.toggle("active");
    });
    language_toggles.forEach(element => {
        element.classList.toggle("en_language");
    });
} );


const service_button = document.querySelector(".services_nav")
service_button.addEventListener("click", (e) => {
    e.preventDefault();
    gsap.to(smoother, {
        scrollTop: smoother.offset(".service", "bottom+=80 bottom"),
        duration: 1,
        ease: "inOut",
    });
});
...
const loadTl = gsap.timeline({
    duration: 1.2,
    onComplete: () => {smoother.paused(false);document.getElementById('video').play;},
}).pause()
...
service_toggles_footer.forEach(element => {
    element.addEventListener("click", (event) => {
        ...
        service_toggles.forEach(element => {
            element.classList.toggle("untoggled_service");
        });
        service_toggles_footer.forEach(element => {
            element.classList.toggle("notactive");
        })
...
function lazyy() {
    ...
    lazy.forEach(element => {
        ...
        var dataSrcValue = element.getAttribute('data');
...
function reloadOnSafari() {
    ...
    if (isSafari) {
        ...
        var isCookieSet = checkCookie('safari-cookie');
        ...
        if (isCookieSet) {
            ...
            var expirationDate = new Date(now.getTime() + 1 * 24 * 60 * 60 * 1000); // 1 day from now

            document.cookie = "safari-cookie=cookie-value; expires=" + expirationDate.toUTCString() + "; path=/";
